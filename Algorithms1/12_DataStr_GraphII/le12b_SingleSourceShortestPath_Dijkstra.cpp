...
void MyGraph::MakeGraphAsAdjacencyList(int n)
{
    ...
    for(int i = 0 ; i < edgeNum ; i++)// 1 origine loop
    {
        int v;//target vertex
        int degree;
        scanf("%d%d",&v, &degree);
        ...
        for(int j = 0 ; j < degree ; j++)//times
        {
            ...
            newNode->u = &G[u];//to original node pointer
...
// function-----
void MyPrim::Dijkstra(int r,int objective)
{
    for(int i = 0 ; i < edgeNum ; i++)// 0 origine
    {
        color[i] = WHITE;
        discover[i] = INT_MAX;
    }
    ...
    while(heapSize != 0)
    {
		...
		if(color[u.id] == WHITE && discover[u.id] > discover[u.id] + u.key)
		{
			discover[u.id] =discover[u.parentId] + u.key;
			color[u.id] = BLACK;
            if(u.id == objective)break;
		}else continue;
        PushAdjaVertix(u.id);
    }
}
...
int main()
{
	...
	for(int i = 0 ; i < pm.edgeNum ; i++)// 1 origine
	{
		pm.Dijkstra(0,i);
        printf("%d %d", i , pm.discover[i]);
        if((i+1) != pm.edgeNum)printf("\n");
	}	
    return 0;
}
